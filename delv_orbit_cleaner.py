...
n_iters = ECI.period*n_orbits/t_burn
size_list = round(n_times/n_iters)

times_lsls = list(chunks(times,size_list))
...
for time in times_lsls:
    ...
    chief = Chief(ECI,time[-1],True)
    s_hat = np.dot(chief.mat,ECI.s_hat)
    #print(s_hat)
    
    pos1 = pert_LVLH_drd1[-1][:3]
    pos2 = pert_LVLH_drd2[-1][:3]
    vel1 = pert_LVLH_drd1[-1][3:]
    vel2 = pert_LVLH_drd2[-1][3:]
...
print("max_sep = %.5f, delv = %.5f"%(max_sep,delv_max))
