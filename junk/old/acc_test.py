...
def worker(ix):
    ...
    for iy in range(len_iy):
        ...
        resulta = np.array([ix,iy,max_acc_s1,max_acc_s2,max_acc_delta_b,delta_v_s1,delta_v_s2,delta_v_delta_b])

    return resulta

p = Pool(processes=30)
resulta = p.map(worker,range(len_ix))
